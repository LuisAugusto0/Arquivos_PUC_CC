...
bool pesquisaBinaria(Jogador* jogador[], char nome[], int qtdeJogadores){
    int esq = 0;
    int dir = qtdeJogadores -1;
    int meio;
    bool tem = false;

    while(esq<=dir){
        meio = (dir-esq)/2 + esq;
        int compNome = strcmp(nome, jogador[meio]->nome); 
        if( compNome < 0 ){ //se nome menor o valor está à esquerda
            dir = meio-1;
            comp++;
        } else if( compNome > 0 ){ //se nome maior o valor está à esquerda
            esq = meio+1;
            comp+=2;
        } else {
            tem = true;
...
int main() {
    ...
    while(strcmp(nome, "FIM")){
        if(pesquisaBinaria(jogador, nome, qtdeJogadores))
            puts("SIM");
        else
            puts("NAO");
        getchar();
    ...
    arq = fopen("matrícula_binaria.txt", "w");
    fprintf(arq, "805413\t%d\t%lf", comp, (double)(end - begin) / CLOCKS_PER_SEC);
