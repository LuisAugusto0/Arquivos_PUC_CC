...
void shellSort(Jogador* jogador[], int tam, int particao){ 
    while(particao > 0){
        for(int grupo=particao; grupo<particao*2; grupo++){ //primeiro for para estabelecer o grupo que a insercao sera feita
            for(int i=grupo; i<tam; i+=particao){ //segundo for para realizar a insercao
                Jogador* tmp = jogador[i];
                int j = i-particao;
            
                while(j>=0 && tmp->peso < jogador[j]->peso){
                    jogador[j+particao] = jogador[j];
                    j-=particao;
                    comp+=2;
                    mov++;
                }
...
int main() {
    ...
    clock_t begin = clock();
    shellSort(jogador, qtdeJogadores, 10);
    clock_t end = clock();
