...
class Q15{
    ...
    private static Jogador[] selecaoParcial(Jogador[] aJogador, int qtdeJogador, int k){
        for(int i=0; i<k; i++){
            int posMenor = i;
    ...
    public static void main(String[] args){
        try{
            ...
            aJogador = selecaoParcial(aJogador, tam, k);
            long fim = new Date().getTime();
            for(int i=0; i<k; i++){
                aJogador[i].imprimir();
            }
            File arq = new File("matricula_selecaoParcial.txt");
